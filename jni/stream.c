...
static void gst_native_init_send_stream (JNIEnv* env, jobject thiz, jstring ip, jstring port){
	...
	strcpy(tmp,"");
	strcat(strcat(strcat(strcat(tmp, "openslessrc ! audioresample ! audio/x-raw, rate=6000, channels=1 ! audioconvert ! speexenc quality=10 ! rtpspeexpay ! udpsink bind-port=8000 name=myudp host="), ip2)," port="),port2);
...
static void gst_native_init_receive_stream (JNIEnv* env, jobject thiz){
	...
	txtPipeline="udpsrc name=myudp caps=\"application/x-rtp, media=(string)audio, clock-rate=(int)6000, encoding-name=(string)SPEEX, encoding-params=(string)1, payload=(int)110\" ! rtpjitterbuffer latency=150 do-lost=true ! rtpspeexdepay ! speexdec ! audioconvert ! audioresample ! openslessink";
	//txtPipeline="udpsrc name=myudp caps=\"application/x-rtp, media=(string)audio, clock-rate=(int)6000, encoding-name=(string)SPEEX, encoding-params=(string)1, payload=(int)110\" ! rtpjitterbuffer latency=50 do-lost=true ! rtpspeexdepay ! speexdec ! audioconvert ! audioresample ! appsink";
